# Pre-AIOS Wiki Log (archived)

**Archived:** 2026-05-23 during wiki conversion to AIOS canonical layout (see [conversion log](2026-05-23-wiki-converted-from-project-root.md) and [import source summary](../sources/2026-05-23-pre-aios-wiki-import.md)).

This file is the verbatim contents of the previous `wiki/log.md` from the project-root llm-wiki that existed 2026-04-24 → 2026-05-23. It is preserved here as historical record. Internal `[link](path)` references inside the entries below assume the **old** layout (`wiki/` at project root, `SCHEMA.md` as the schema doc, single `log.md` file). Many of those links are now broken; the current canonical schema lives at [../WIKI-CLAUDE.md](../WIKI-CLAUDE.md) and all subsequent log events are filed as `docs/wiki/log/YYYY-MM-DD-<slug>.md` files alongside this one.

Format used by these entries: `## [YYYY-MM-DD] type | short title` where `type` is one of `ingest`, `decision`, `query`, `lint`, `edit`.
